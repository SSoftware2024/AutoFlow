    ...
    <AuthenticationCard>
        ...
        <div class="mb-4 text-sm text-gray-600 dark:text-neutral-50">
            <template v-if="! recovery">
                Confirme o acesso à sua conta inserindo o código de autenticação fornecido pelo seu aplicativo Authenticator.
            </template>
        ...
        <form @submit.prevent="submit">
            ...
            <div v-else>
                <InputLabel for="recovery_code" value="Código de recuperação" class="dark:text-neutral-50"/>
                <TextInput
                    id="recovery_code"
            ...
            <div class="flex items-center justify-end mt-4">
                <button type="button" class="text-sm text-gray-600 underline cursor-pointer hover:text-gray-900" @click.prevent="toggleRecovery">
                    <template v-if="! recovery">
                        Use código de recuperação
                    </template>

                    <template v-else>
                        Use código de aunteticação
                    </template>
                </button>
