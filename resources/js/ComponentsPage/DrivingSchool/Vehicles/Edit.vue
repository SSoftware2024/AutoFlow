    ...
    <form @submit.prevent="save">
        ...
        <div class="row sm:space-x-1">
            <div class="row-col">
                ...
                <Calendar v-model="form.ipva_generate" dateFormat="dd/mm/yy" :minDate="new Date()" showIcon
                    iconDisplay="input" :class="{
...
const form = useForm({
    surname: page.props.vehicle.surname,
    plate: page.props.vehicle.plate,
    category: page.props.vehicle.category,
    ipva_value: page.props.vehicle.ipva_value,
    ipva_generate: new Date(page.props.vehicle.ipva_generate) //data ipva
});
...
function _infoIPVAdate() {
    alert.info("Na edição caso IPVA diferente do inserido o sitema que mudou automaticamente, não faça a atualização desse valor, conquanto caso IPVA tenha mudado data original faça a mudança. <br/> A mudança afeterá no geramento automático de contas a pagar.");
}

function _checkModelPlateMounted() {
    model_plate.value = page.props.vehicle.plate.length == 8 ? 0 : 1;
}


function save() {
    form.transform((data) => ({
        id: page.props.vehicle.id,
        ...data
    })).put(route('user.driving_school.vehicles.update'));
}

onMounted(() => {
    _checkModelPlateMounted();
});
</script>
